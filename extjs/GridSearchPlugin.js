...
Ext.extend(Ext.ux.grid.Search, Ext.util.Observable, {
    ...
    ,onTriggerSearch:function() {
        ...
        if(this.paramFields)
        {
            Ext.each(this.paramFields,function(item){
                    delete(store.baseParams[item]);
                    if (store.lastOptions && store.lastOptions.params) {
                        delete(store.lastOptions.params[item]);
                    }
            });
        }
        ...
        // ask server to filter records
            ...
            this.paramFields = [];
            var paramfields = this.paramFields;
            var fields = [];
            this.menu.items.each(function(item) {
                if(item.checked) {
                    fields.push(item.dataIndex);
                    
                    var where = 'where|' + item.dataIndex;
                    paramfields.push(where);
                    var value = val;
                    if(value.indexOf(item.dataIndex)==-1)
                    {
                        //value = item.dataIndex + ' like $%' + value + '%$';
						value =  value;
                    }
                    else
                    {
                        value = item.dataIndex + ' = $' + value + '$';
                    }
                    delete(store.baseParams[where]);
                    if (store.lastOptions && store.lastOptions.params) {
                        delete(store.lastOptions.params[where]);
                    }
                    
                    if(val)
                    {
                        store.baseParams[where] = value;
                    }
                }
            });
        ...
        Ext.each(cm.config, function(config) {
            ...
            if(config.header && config.dataIndex) {
                Ext.each(this.disableIndexes, function(item) {
                    if(config.header==item || config.dataIndex==item)
                    {
                        disable = true;
                    }
                    else if(config.dataIndex.indexOf('|' + item)>=0)
                    {
                        disable = true;
                    }
                });
